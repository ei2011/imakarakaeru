...
//緯度、経度は少数だから0.0で初期化
	...
	override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
		//ACCESS_FINE_LOCATIONに対するパーミション（アクセス権）ダイアログでかつ許可を選択したなら…  p329詳しくは
		if(requestCode == 1000 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
			//LocationManagerオブジェクトを取得。
			val locationManager = getSystemService(Context.LOCATION_SERVICE) as LocationManager
			//位置情報が更新された際のリスナオブジェクトを生成。
			//getSystemService()は親クラスのcontextクラスのメゾットでosレベルで提供しているサービスのオブジェクトを取得する
			//LOCATION_SERVICEは引数
	...
	fun onMapSearchButtonClick(view: View) {
		...
		//findViewById  は　変数　etSearchWord　に　etSearchWord　の参照する場所をしめしている感じ
		//view型　の　view　という変数が引数となる
		val etSearchWord = findViewById<EditText>(R.id.etSearchWord)
		var searchWord = etSearchWord.text.toString()
		//入力されたキーワードをURLエンコード。
		//searchWord のtext部分をストリング型にキャスト
		searchWord = URLEncoder.encode(searchWord, "UTF-8")
		//マップアプリと連携するURI文字列を生成。
		val uriStr = "geo:0,0?q=${searchWord}"
		//URI文字列からURIオブジェクトを生成。
		//"geo:0,0?q=${文字列}" とすることで地図アプリがその文字列で検索する
		val uri = Uri.parse(uriStr)
		//Intentオブジェクトを生成。
		//(Intent.ACTION_VIEW, uri) intenという変数に（すること.なにをかえすか）の情報をまとめたものを格納
		//parse() で解析をして、
